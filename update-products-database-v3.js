...
function updateAccessoryCategories() {
    // 清空配件表
    connection.query('DELETE FROM accessories', (err) => {
        if (err) {
            console.error('清空配件表失败:', err);
            connection.end();
            return;
        }
        console.log('已清空配件表');
...
function addForeignKeys() {
    ...
    connection.query(`
        ...
        ADD CONSTRAINT fk_product_accessories_product 
        FOREIGN KEY (product_id) REFERENCES products(id) ON DELETE CASCADE,
        ADD CONSTRAINT fk_product_accessories_accessory 
        FOREIGN KEY (accessory_id) REFERENCES accessories(id) ON DELETE CASCADE,
        ADD UNIQUE KEY (product_id, accessory_id)
    `, (err) => {
        if (err && err.code !== 'ER_DUP_KEYNAME' && err.code !== 'ER_CANNOT_ADD_FOREIGN') {
            console.error('添加产品配件关联表外键失败:', err);
        } else {
            console.log('已添加产品配件关联表外键');
        }
