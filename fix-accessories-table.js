...
// 检查表结构
connection.query('DESCRIBE accessories', (err, columns) => {
    if (err) {
        console.error('Error describing accessories table:', err);
        connection.end();
        return;
    }
    
    const columnNames = columns.map(col => col.Field);
    console.log('当前配件表列:', columnNames);
    
    // 需要添加的列
    const requiredColumns = [
        { name: 'model', definition: "VARCHAR(255) COMMENT '型号规格'" },
        { name: 'brand', definition: "VARCHAR(100) COMMENT '品牌'" },
        { name: 'purchase_price', definition: "DECIMAL(10,2) COMMENT '采购价格'" },
        { name: 'purchase_date', definition: "DATE COMMENT '采购时间'" },
        { name: 'stock_quantity', definition: "INT DEFAULT 0 COMMENT '库存数量'" },
        { name: 'status', definition: "ENUM('assembled', 'in_warehouse', 'scrapped') DEFAULT 'in_warehouse' COMMENT '状态：已组装，在仓库，报废'" }
    ];
    ...
    requiredColumns.forEach(col => {
        if (!columnNames.includes(col.name)) {
            alterCount++;
            const sql = `ALTER TABLE accessories ADD COLUMN ${col.name} ${col.definition}`;
            console.log('添加列:', sql);
...
function createOtherTables() {
    ...
    createTableStatements.forEach(sql => {
        console.log('创建表:', sql.substring(0, 50) + '...');
        connection.query(sql, (err) => {
            if (err) {
            ...
            completedCount++;
            if (completedCount === createTableStatements.length) {
                // 插入默认设置
...
function insertDefaultSettings() {
    ...
    // 检查是否已有设置，没有则插入
    connection.query('SELECT COUNT(*) as count FROM accessory_settings', (err, result) => {
        if (err) {
        ...
        if (result[0].count === 0) {
            ...
            defaultSettings.forEach(setting => {
                connection.query(
                    ...
                    (err) => {
                        if (err) {
                            console.error(`插入设置 ${setting[0]} 失败:`, err);
                        } else {
                            console.log(`插入设置 ${setting[0]} 成功`);
                        }
                        
                        completedCount++;
                        if (completedCount === defaultSettings.length) {
